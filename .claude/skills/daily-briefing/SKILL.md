...
2. **일정 읽기 (폴백 규약 — 순서 고정):**
   - ① 구글 캘린더 **실연동**(`state/google-calendar.json`)이 있으면 그것을 읽는다.
   - ② 없으면 **ICS 비밀 주소**(`state/calendar.json`)를 읽는다.
   - ③ 둘 다 없거나 실패하면 **`notes/calendar-fallback.md`** 를 읽는다.
   - 이 순서는 브리지의 `/api/calendar` 와 **같다** — 브리핑과 대시보드가 같은 일정을 본다.
