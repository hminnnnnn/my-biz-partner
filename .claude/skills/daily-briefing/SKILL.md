---
name: daily-briefing
description: "일정 브리핑과 미팅 사전 준비. '내일 일정 브리핑해줘', '오늘 뭐 있지?', '이번 주 일정 알려줘', '15시 미팅 사전 준비 챙겨줘', '아침 브리핑' 처럼 일정·미팅 준비를 물을 때 사용한다. 캘린더(또는 폴백 파일)와 기록을 엮어 짧게 브리핑한다."
---
...
2. **일정 읽기 (폴백 규약 — 순서 고정):**
   ...
   > **읽기 전용이다.** 일정을 만들거나 고치거나 지우는 건 화면(대시보드)만 한다.
   > 상대가 "일정 잡아줘" 라고 하면 — 없는 기능을 있다고 하지 말고 —
   > *"일정 만들기는 대시보드에서 날짜를 누르시면 돼요. 저는 읽기만 합니다"* 라고 안내한다.
   > (실측 2026-08-05: 이 창구가 없어서 브리핑이 매번 "일정을 못 읽었습니다"로 끝났다.
   >  2026-08-07 에 `tools/calendar-read.mjs` 로 메웠다.)
