...
2. **일정 읽기 (폴백 규약 — 순서 고정):**
   - ① **ICS 비밀 주소**(`state/calendar.json`)가 있으면 그 주소를 받아 읽는다.
   - ② 없거나 실패하면 **`notes/calendar-fallback.md`** 를 읽는다.

   > **구글 실연동 일정은 여기서 못 읽는다 — 사실대로 말한다.**
   > `state/google-calendar.json` 에는 열쇠만 들어 있고, 실제 일정을 받으려면 구글 API 를
   > 인증 호출해야 하는데 **이 폴더의 권한에는 그 수단이 없다**(`curl` 은 거부 목록에 있고,
   > 호출용 스크립트도 허용돼 있지 않다). 구글 일정을 실제로 읽는 건 **대시보드 서버뿐**이다
   > (`dashboard-server.py` 가 파이썬으로 호출한다).
   >
   > 그러니 구글만 연결돼 있고 ICS·폴백이 비어 있으면 **일정 칸을 지어내지 말고 이렇게 말한다**:
   > *"구글 캘린더 일정은 제가 직접 못 읽어요 — 대시보드(`./dashboard.sh`)를 열면 거기서는
   > 보입니다. 텔레그램으로도 받고 싶으시면 `notes/calendar-fallback.md` 에 이번 주 일정을
   > 붙여넣어 주세요."*
   > (실측 2026-08-05: 이 안내 없이 "①구글을 읽는다"고만 적혀 있어서, 브리핑이 매번
   >  "일정을 못 읽었습니다"로 끝났다.)
