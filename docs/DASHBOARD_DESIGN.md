...
> 2026-07-28 기준 — GYEOL 전면 개편(7/23) + 팀 탭(W17) + 기록 탭 개편(W18) + W20 전면 개편 +
> W21 작업 공간 전환 + W22 올인원 전환 + **W23 React 재구축(파일 모드 폐지·서버 단일·실증 스위트·
> ⌘K 통합 팔레트·시드 점프·j/k·react-table)** 누적 반영.
...
3. 끝나면 **`tools/verify-dashboard.sh` 전 게이트 PASS** 를 확인한다 (개발 레포에서만 — 스위트는
   ...
   3키트**(`tools/dash-e2e/run.mjs` — 실브라우저 클릭·입력·네트워크 캡처, P-A 60 · P-B 75 ·
   P-C 72) → 브리지 API 23 → **테스트 고아 0**.
...
4. 시각 변경은 **스크린샷 육안 검수**를 추가한다: `node tools/dash-e2e/shots.mjs <출력폴더> [키트]`
   (라이트/다크 × 7탭). DOM 단언은 CSS가 안 떠도 통과할 수 있다 — 실제 사고 사례 있음.
