...
> **W20 전면 개편(셸 레이어·탭별 심화·서버 모드)** 누적 반영.
...
3. 끝나면 아래 **둘 다** 전항 PASS 를 확인한다 (개발 레포에서만 — 스위트는 배포물 밖 `tools/` 에 있다):
   - `tools/dashboard-e2e/run.sh [키트]` — **file:// 모드**. 세 키트 전부 통과해야 한다:
     `demo/P-A_마중물컨설팅`(빈 데이터) **62** · `demo/P-B_별빛담은`(풀 데이터) **91** ·
     `demo/P-C_모먼트`(소집 중) **88**.
...
4. `tools/dashboard-e2e/run.sh` 세 키트 전항 PASS (P-A 62 · P-B 91 · P-C 88) +
   `tools/dashboard-e2e/run-server.sh` 전항 PASS (94 + 87)
