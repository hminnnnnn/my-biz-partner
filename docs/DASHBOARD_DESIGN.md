...
> **빌드 산출물**이라 직접 고치지 않는다. 구 대시보드는 `dashboard-legacy/`(롤백용)에 보존.
> 데이터 규약은 `STATE_CONTRACT.md`, 디자인 시스템 상세는 gyeol-design-system 스킬이 기준.
> 2026-07-30 기준 — GYEOL 전면 개편(7/23) + 팀 탭(W17) + 기록 탭 개편(W18) + W20 전면 개편 +
> W21 작업 공간 전환 + W22 올인원 전환 + W23 React 재구축(파일 모드 폐지·서버 단일·실증 스위트·
> ⌘K 통합 팔레트·시드 점프·j/k·react-table) + **W26 UX 재설계(6탭 — 프로젝트 탭 삭제 ·
> 홈 다섯 질문 · 소유권 분리(할 일=사장/칸반=팀 티켓) · 팀 로스터+작업 로그 · 자동화 등록만 ·
> 대화 도크 · 캘린더 연동)** 누적 반영. W26 의 대표 발언 원문·근거는
...
3. 끝나면 **`tools/verify-dashboard.sh` 전 게이트 PASS** 를 확인한다 (개발 레포에서만 — 스위트는
   배포물 밖 `tools/` 에 있다). 게이트 6단계: 타입 검사 → 빌드 → 키트 동기화 → **실증 스위트
   3키트**(`tools/dash-e2e/run.mjs` — 실브라우저 클릭·입력·네트워크 캡처, P-A 63 · P-B 78 ·
   P-C 75) → 브리지 API 23 → **테스트 고아 0**.
   기대값은 전부 **키트 데이터에서 파생**한다 — 데이터명을 하네스에 박제하지 말 것
   (다른 키트에서 허위 FAIL 10건이 났던 전력).
   구 스위트(`tools/dashboard-e2e/`)는 `dashboard-legacy/` 를 만질 때만 쓴다.
...
- 키보드만으로 6탭 이동(`1`~`6`)·검색(`⌘K`/`/`)·모달 닫기(`ESC`)가 된다.
...
node tools/dash-e2e/shots.mjs /tmp/s   # 라이트/다크 × 6탭 캡처 (시각 변경 시 육안)
```
...
3. 시각 변경 시 `node tools/dash-e2e/shots.mjs <저장폴더> <키트>` 로 라이트+다크 × 6탭 육안.
