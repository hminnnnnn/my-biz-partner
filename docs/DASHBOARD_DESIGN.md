...
3. 끝나면 **`tools/dashboard-e2e/run.sh` 를 실행해 28/28 PASS** 를 확인한다(개발 레포에서만 —
...
- **하드코딩 금지**: UI 코드에 hex·px·radius 값 직접 쓰지 않는다. 전부 `--g-*` 토큰.
  (허용된 예외: 반응형 브레이크포인트, 1px 헤어라인, 상태점 등 장식 고정치수 — 색은 예외 없음)
...
- **정직**: 제품(스킬)에 존재하지 않는 기능·개념을 화면 문구로 약속하지 않는다.
  (실사례: "이름·성향을 가진 8명 채용" 문구가 리뷰에서 반려됨 — 스킬에 없는 서사였다)

## 3. 보존 계약 (제거·개명 금지 — e2e 스위트가 이 훅으로 검증한다)

- 탭: `#home #todos #project #kanban #records` 5탭. IA 변경은 대표 승인 필요.
- 데이터 훅: `.todo-row` `.todo(.open)` `[data-exp]` `[data-viewer]` `[data-goto]` `[data-copy]`
  ...
  `#modal #modal-title #modal-body` `.role-card` `.proj-card` `.decide`.
