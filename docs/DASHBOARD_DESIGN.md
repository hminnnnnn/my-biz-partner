...
4. 시각 변경은 **스크린샷 육안 검수**를 추가한다: `node tools/dash-e2e/shots.mjs <출력폴더> [키트]`
   (라이트/다크 × 6탭). DOM 단언은 CSS가 안 떠도 통과할 수 있다 — 실제 사고 사례 있음.
...
- **심플 이즈 더 베스트 (W26 — 대표 지시)**: 화면은 "진짜 필요한 내용·진짜 사용할 기능"만 담는다.
  ...
  홈은 **다섯 질문**에만 답한다: 챙겨야 할 것 · 지금 해야 할 일 · 놓친 것 · 앞으로의 계획 ·
  전체 일정(캘린더). 새 정보를 추가하려면 이 질문 중 어디에 답하는지 먼저 말할 수 있어야 한다.
- **소유권 분리 (W26)**: 할 일 탭 = **사장이 챙길 것**, 칸반 = **팀원 작업 티켓**(맥락 포함).
  두 모집단을 한 화면에 섞지 않는다 — "누가 해야 할 일인지"가 첫눈에 갈려야 한다.
- **판단 우선 위계**: 위급한 것이 위에 온다 — 홈에서 '놓친 것'이 최상단. 선정 규칙은 전부
  데이터 파생. 상수로 지어낸 사유·문구로 긴급함을 연출하지 않는다.
