...
대시보드는 **두 가지 모드**로 읽는다. 어느 쪽이든 쓰기 방향은 그대로다(에이전트 → 파일).

| 모드 | 어떻게 열었나 | 무엇을 읽나 |
|---|---|---|
| **파일 모드** | `dashboard/index.html` 더블클릭 (`file://`) | `state/dashboard-data.js` **만**. `file://` 은 CORS 로 `fetch` 가 막혀 래퍼가 유일한 통로다 |
| **서버 모드** | `./dashboard.sh` (`http://127.0.0.1`) | `state/status.json`·`state/team.json` 을 **직접**, 그리고 `records`(노트 목록·트리)는 래퍼에서. 노트 본문은 필요할 때 파일에서 직접 |

- **파일 모드는 항상 유지한다** — 서버 없이 더블클릭만으로도 지금까지와 동일하게 동작해야 한다.
  서버 모드에서 직독이 실패하면 조용히 래퍼 값으로 폴백하고, 화면 아래에 그 사실을 표시한다.
- 서버 모드에서도 **`refresh-dashboard.sh` 는 여전히 필요하다** — `records`(노트 목록·폴더 트리)와
  `roster` 는 파일시스템 스캔 결과라 브라우저가 만들 수 없다. 다만 `status`·`team` 은 직독하므로
  **래퍼가 낡아도 홈·할 일·프로젝트·팀·칸반은 최신으로 보인다.**
- 갱신은 **변경 감지식**이다(15초 주기로 확인, `updatedAt` 이 실제로 바뀐 경우에만 반영).
  파일 모드는 리로드, 서버 모드는 리로드 없이 제자리 갱신(스크롤·펼침 유지).
